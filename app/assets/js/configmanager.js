...
const dataPath = path.join(sysRoot, '.acrplauncher')

function getLauncherDir() {
    try {
        return require('@electron/remote').app.getPath('userData')
    } catch {
        return path.join(os.homedir(), '.acrplauncher')
    }
}
...
exports.getTermsAccepted = function() {
    return config.termsAccepted === true
}

exports.setTermsAccepted = function(val) {
    config.termsAccepted = val === true
}
...
/**
 * Adds an offline account for servers configured to accept offline profiles.
 * The UUID matches Minecraft's deterministic OfflinePlayer name UUID format.
 *
 * @param {string} displayName The in-game player name.
...
exports.addOfflineAuthAccount = function(displayName){
    ...
    const uuid = digest.toString('hex')
    config.selectedAccount = uuid
    config.authenticationDatabase[uuid] = {
        type: 'offline',
        accessToken: '0',
