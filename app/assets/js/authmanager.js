...
/**
 * Add an offline profile. Identity protection for these profiles must be
 * enforced by the game server because no Microsoft token is available.
 *
 * @param {string} displayName The in-game player name.
 * @returns {Object} The stored offline account.
 */
exports.addOfflineAccount = function(displayName) {
    const ret = ConfigManager.addOfflineAuthAccount(displayName)
    ConfigManager.save()
    return ret
}
