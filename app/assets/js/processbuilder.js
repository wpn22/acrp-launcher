...
class ProcessBuilder {
    ...
    constructor(distroServer, vanillaManifest, modManifest, authUser, launcherVersion){
        ...
        this.usingExplicitForgeModList = false
        this.llPath = null
    }
    ...
    build(){
        ...
        this.usingExplicitForgeModList = modObj.fMods.length > 0
        
        // Mod list below 1.13
        // Fabric only supports 1.14+
        if(!mcVersionAtLeast('1.13', this.server.rawServer.minecraftVersion)){
            if(this.usingExplicitForgeModList) {
    ...
    /**
     * Locate instance and Java native folders required by embedded browser mods.
     */
    _resolveNativeDependencyPaths() {
        const javaBin = path.dirname(ConfigManager.getJavaExecutable(this.server.rawServer.id))
        const jreBin = path.join(path.dirname(javaBin), 'jre', 'bin')
        return [
            path.join(this.gameDir, 'jcef'),
            path.join(this.gameDir, 'jcef', 'bin'),
            path.join(this.gameDir, 'jcef', 'swiftshader'),
            javaBin,
            path.join(javaBin, 'server'),
            jreBin,
            path.join(jreBin, 'server')
        ].filter((value, index, values) => fs.existsSync(value) && values.indexOf(value) === index)
    }

    /**
     * Add per-instance native folders to the launched game process.
     * MCEF/JCEF ships Windows DLL dependencies next to jcef.dll, and Windows
     * needs those folders on PATH while Minecraft is running.
     */
    _resolveProcessEnv() {
        const env = Object.assign({}, process.env)
        const nativePaths = this._resolveNativeDependencyPaths()
        ...
        if(nativePaths.length > 0) {
            ...
            env[pathKey] = nativePaths.concat(currentPath ? [currentPath] : []).join(path.delimiter)
            logger.info('Added native dependency paths:', nativePaths)
        }
    ...
    _constructJVMArguments112(mods, tempNativePath){
        ...
        if(process.platform === 'darwin'){
            args.push('-Xdock:name=ACRP Launcher')
            args.push('-Xdock:icon=' + path.join(__dirname, '..', 'images', 'minecraft.icns'))
        }
        args.push('-Xmx' + ConfigManager.getMaxRAM(this.server.rawServer.id))
        args.push('-Xms' + ConfigManager.getMinRAM(this.server.rawServer.id))
        args = args.concat(ConfigManager.getJVMOptions(this.server.rawServer.id))
        args.push('-Djava.library.path=' + [tempNativePath].concat(this._resolveNativeDependencyPaths()).join(ProcessBuilder.getClasspathSeparator()))
    ...
    _constructJVMArguments113(mods, tempNativePath){
        ...
        //args.push('-Dlog4j.configurationFile=D:\\AdventureCity\\game\\common\\assets\\log_configs\\client-1.12.xml')
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'ACRP-Launcher')
                            break
    ...
    _resolveForgeArgs(){
        ...
        // ACRP keeps protected mods in the instance mods folder with their original names.
        if(this.usingExplicitForgeModList) {
